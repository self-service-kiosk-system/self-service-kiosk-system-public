...
const AgregarProducto = () => {
  ...
  // Cargar el local del usuario al montar
  useEffect(() => {
    cargarLocalUsuario();
  }, []);

  // Cargar categorías cuando tengamos el local_id del usuario
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('local_id', formData.local_id);
      ...
      if (response.ok) {
        ...
        const data = await response.json();
        setError(data.detail || 'Error al crear producto');
      }
    } catch (err) {
        ...
        <div className="form-row">
          <div className="form-group">
            ...
            <input
              type="text"
              id="local_id_display"
              value={cargandoLocal ? 'Cargando...' : (localUsuario?.nombre || 'Sin local asignado')}
              disabled
            ...
            />
            {!localUsuario && !cargandoLocal && (
              <span className="error-campo">No tienes un local asignado</span>
            )}
          </div>
